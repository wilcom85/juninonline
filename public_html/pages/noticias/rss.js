...
function ReadRSS(rssFeed, Body, Title) 
{
	...
	try
	{
		if (document.all)
		{
			var errorHappendHere = "Por favor verifique su Navegador y los permisos de seguridad";
			xmlDoc = new ActiveXObject("Microsoft.XMLDOM");
		}
		else
		{
			var errorHappendHere = "Actualmente el servicio de \n\
                                                lectura de noticias solo funciona en internet explorer estamos trabajando para soportar otros navegadores";
	...
	{
		rssTitle.innerHTML = 'Ocurrio un error';
		rssBody.innerHTML = '   Error:'+e.message+"<br/>Note: "+errorHappendHere;
	}
}
...
function SetRSSTemplates()
{
	if (rssBody)
	{
		...
		for(var i=0; i< items.length; i++) 
		{
			...
			output = Replace(output,"(::Description::)",getNode('content:encoded',i));
			buffer+=output;
		}
